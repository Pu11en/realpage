...
def _build_record(
    ...
    name = _clean_name(str(row.get(name_key) or ""), recipe or {}) if name_key else ""
    if not name:
...
def _clean_name(name: str, recipe: dict | None = None) -> str:
    """The project name as a seller would recognise it.

    A recipe can strip city filing prefixes, and reviewer routing tags are
    stripped when the feed writes them as "reviewer /// project".
    """
    recipe = recipe or {}
    pattern = recipe.get("name_strip_pattern")
    if pattern and name:
        name = re.sub(pattern, "", name, count=1, flags=re.I).strip(" -")
