...
def test_named_blank_address_is_kept_but_placeholder_null_is_dropped():
    """A genuinely blank address can still be a named project, but a literal
    placeholder such as NULL is not a seller-callable street address."""
    recipe = {
        "endpoint": "https://example.test/sql",
        "fields": {"permit_type": "Name", "name": "Name", "issue_date": "Issued", "address": "Addr"},
    }
    ...
    stats = {}
    records = find_upcoming("Rivertown", "ZZ", "zz", recipe, _http_get(rows), today=TODAY, stats=stats)
    assert [(r.name, r.address) for r in records] == [("AAA APARTMENTS", ""), ("CCC APARTMENTS", "7 Real St")]
