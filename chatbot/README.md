...
| Path | What |
|---|---|
| `Dockerfile` | Build context = repo root. Bakes `propertystack/data/plano-richardson/*.csv` and the other lead data in read-only. |
...
| `hermes-profile/plugins/propertystack/` | Tools: `ps_schema`, `ps_sql` (single SELECT, read-only SQLite + authorizer). CSVs load into SQLite at startup. |
