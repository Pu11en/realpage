...
| `cranesignal_chat_stats`, `cranesignal_eval_checks`, `cranesignal_eval_summary`, `cranesignal_eval_failure_types`, `cranesignal_buildbot_summary`, `cranesignal_buildbot_examples` | CraneSignal measurements | Chat timing, eval results, failure types, and AI build-agent progress from the Under the Hood page. These rows have `measured_at` dates; state the date because eval numbers may be old. |
| `ai_visibility_summary`, `ai_visibility_models`, `ai_visibility_competitors`, `ai_visibility_questions`, `ai_visibility_top_picks` | AI Visibility score snapshot | The read-only AI Visibility page data for RealPage: overall score, model scores, competitor scores, top picks, and individual question outcomes. Use only for questions about RealPage's visibility in AI answers. State `generated_at` because it is a point-in-time score. |
| `ai_visibility_actions`, `ai_visibility_site_facts`, `ai_visibility_caveats` | AI Visibility recommended actions | Recommended website/content actions and caveats from the AI Visibility actions file. Use with the score tables when asked what RealPage should fix for AI answers. State `based_on` because the recommendations depend on the tested dates. |
