...
## S5 Score + cards + map markers — done (2026-09-13)
- run.py now writes data/scout/<date>/areas.json (numbers + evidence links only), cards.md (top 5, "why this city" read from why.json that the local Claude session writes), refreshes kind:"scout" markers (top 5) in site/data/reach.json (old scout markers replaced, reach dots kept, file created if missing), and logs runs/<ts>-scout-areas.json via lib.runlog. New `--from-saved <date>` rescoring rebuilds from saved sample.csv + evidence.json with zero searches.
- Wrote the Tucson why.json (3 lines citing the Yardi/Entrata proof URLs, Census numbers, 2026 sales). Tucson: growth 65, churn 15, pain 34, total 38.
- Commit: 7ab9396. Checked: pytest -> 27 passed (5 new, offline); live `--from-saved 2026-09-13 --metros tucson-az` -> 0 searches, all 3 outputs written.
- Open: site/data/reach.json doesn't exist on this branch yet (the map-deepdive branch creates it with 19 reach dots) — merging will conflict on that file; resolve by taking theirs and rerunning `run.py --from-saved <date>` to re-add scout markers. Marker links show vendor proof first (up to 5), so sale links may be cut off.
