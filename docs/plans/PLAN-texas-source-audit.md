...
- [ ] T9 Narrow the routine run to Texas. `tooling/new-run.sh` with no argument runs Texas only; Arizona and New Mexico keep their existing site data and are re-run only when named explicitly. Make sure the site still shows Arizona and that the totals do not drop. Tests.
