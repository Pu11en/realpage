...
Build the ~25 pages specified in `PLAN-seo-geo-strategy.md` — metro pipeline reports first, then the
two state pages, then the big cities, then the two listicles. Read that file before starting; it
carries the search-demand evidence, the Google-2026 thin-page limits, and the explicit do-not-build
list. Generated by extending `site/data/build_data.py` (not a second builder), served from
`/leads/*` via `site/Caddyfile`, listed in the sitemap, tested for unique titles, real row counts,
working source links and no RealPage text. Commit after each tier.
