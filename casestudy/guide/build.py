...
("Customer replies and tricky cases", [
 ...
 ("What if a record isn't about booking a tour (rent, maintenance, renewal, applications)?", "It sends a short, general, checked message using the record's own goal as the button (like 'review renewal'), on the right channel, at the right time, with consent. For example: 'Hi Alex, a quick note from Oak Ridge about your lease renewal. Reply here and our team will help.' It never sends a tour pitch for a different job. The AI isn't used for these yet, because there's no approved playbook, so the checked template is the whole answer.", ["Before the fix, a rent reminder got a tour invite. Now the message matches the task.", "Next step: a real template per task type, with its own checks."]),
