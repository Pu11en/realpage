...
("Customer replies and tricky cases", [
 ...
 ("What if a record isn't about booking a tour (rent, maintenance, renewal, applications)?", "It goes to a person. The only approach the two examples teach is prospect tour outreach, so rather than send a tour pitch for rent or maintenance, the agent escalates with the reason 'no approved approach for this task'. Adding a new task type means a new template plus its checks; the permission rules and safety check stay the same.", ["Found in testing: before the fix, a rent reminder got a tour invite."]),
