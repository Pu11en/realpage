...
**NOT approved to run yet** -- Drew is still planning it. Runs after `PLAN-client-map.md` (done).
...
- **Run size:** whole state, city by city, until **150 new projects** or **~450 Jina searches**.
...
- **Tools (all local):** this Claude session, crawl4ai (`localhost:11235`), Playwright + stealth,
  Jina (only paid, capped), Claude web search/fetch, `tooling/pms_detect.py`, Census data, optional
...
- [ ] **L1 Skeleton, caps, no-place-names test.** `propertystack/skills/lead-finder/` (`SKILL.md`
  ...
  150-project and 450-search caps stop cleanly, lead ranking order, and a test that **fails if any
...
- [ ] **L4 `find-upcoming` rebuilt for any city.** Replace the old Plano-only version: given a city
  and its recipe, pull new apartment permits (permit issued → leasing), one row per project with
  its permit link. No hard-coded searches or places. Fixture tests. Commit.
